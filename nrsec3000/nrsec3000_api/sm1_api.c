...
HD_U8 hd_sm1_import_key(HD_U32 fd, HD_U8 *txbuf)
{
	HD_U8 insbuf[1];
	HD_U8 swbuf[1];
	HD_U8 ek_crc[1];
	...
	hd_spi_write(fd, txbuf, rxbuf, HD_SM1_KEY_LEN);
	hd_spi_write(fd, ek_crc, rxbuf, 1);
	...
	hd_spi_write(fd, akbuf, rxbuf, HD_SM1_KEY_LEN);
	hd_spi_write(fd, ak_crc, rxbuf, 1);
...
HD_U8 hd_sm1_import_iv(HD_U32 fd, HD_U8 *txbuf)
{
	HD_U8 cmd_buf[HD_SM_CMD_LEN];
	HD_U8 rxbuf[HD_SM_RXBUF_LEN];
	HD_U8 insbuf[1];
	...
	hd_spi_write(fd, txbuf, rxbuf, HD_SM1_IV_LEN);
	hd_spi_write(fd, iv_crc, rxbuf, 1);
...
HD_U8 hd_sm1_encrypt(HD_U32 fd, HD_U8 *key_buf, HD_U8 *iv_buf, HD_U8 *txbuf, HD_U32 tx_len, HD_U8 *rxbuf, HD_U32 *rx_len)
{
	HD_U8 cmd_buf[HD_SM_CMD_LEN];
	...
	re_val = hd_sm1_import_key(fd, key_buf);
	if(re_val != HD_SM_OK)
		return re_val;
	
	re_val = hd_sm1_import_iv(fd, iv_buf);
	if(re_val != HD_SM_OK)
		return re_val;
	...
	{
		printf("hd_sm1_encrypt rxcrc err!\n");
...
HD_U8 hd_sm1_decrypt(HD_U32 fd, HD_U8 *key_buf, HD_U8 *iv_buf, HD_U8 *txbuf, HD_U32 tx_len, HD_U8 *rxbuf, HD_U32 *rx_len)
{
	HD_U8 cmd_buf[HD_SM_CMD_LEN];
	HD_U8 rx_buf[HD_SM_RXBUF_LEN];
	HD_U8 insbuf[1];
	...
	if(rx_crc[0] != rxbuf[rxlen - 1])
	{
		printf("hd_sm1_decrypt rxcrc err!\n");
		return HD_SM_RX_CRC_ERR;
	}
	rx_len[0] = rxlen - 1;
